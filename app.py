...
def main() -> None:
    ...
    st.markdown(
        """
        <div class="hero">
            <h1>Excel Automation & AI Reporting Tool</h1>
            <p>Upload a spreadsheet to automatically clean the data, generate pivot-table
            analytics, build an interactive dashboard, surface AI-style insights, and export
            client-ready Excel, PDF, and PowerPoint reports.</p>
        </div>
        """,
    ...
    with tabs[1]:
        ...
        st.markdown('<div class="section-title">Plain-Language Overview</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="insight">{ai["dataset_summary"]}</div>', unsafe_allow_html=True)
    ...
    with tabs[2]:
        ...
        render_metric_cards(kpis)
        st.caption("Use the sidebar filters to slice the dashboard by category and date range.")
        render_charts(charts)

    # --- Tab 4: Pivot Tables ---
    with tabs[3]:
        st.markdown('<div class="section-title">Automated Pivot Tables</div>', unsafe_allow_html=True)
        if not pivots:
    ...
    with tabs[4]:
        ...
        st.markdown(
            f'<div class="section-title">AI Business Insights '
            f'<span style="font-size:.8rem;color:#64748b;">({type_label} data · {badge})</span></div>',
            unsafe_allow_html=True,
        )
        ...
        for item in ai["anomalies"]:
            ...
            css = "insight" if stable else "insight warning"
